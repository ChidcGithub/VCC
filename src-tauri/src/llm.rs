...
fn tool_defs() -> Vec<Value> {
    ...
    vec![
        ...
        tool("ocr_screen", "识别屏幕/窗口里的文字和图片内容（你没有视觉能力：用户问屏幕上/图片里/PPT/视频画面有什么，必须先调这个识别实际像素，绝不回答「我看不到」。整屏识别不传 window）", json!({
...
3. 多个操作按顺序逐个调用工具。\n\
4. 你没有任何视觉能力，看不到截图/图片/照片/PPT画面/视频帧。用户想知道屏幕上、窗口里或图片里有什么内容时，必须先调工具实际查看再回答：文字与图片内容用 ocr_screen（识别像素文字），界面控件布局用 read_screen（可点击元素+坐标），必要时两者都调；拿到结果前绝不回答，也绝不说「我看不到/无法识别图片」。鼠标坐标是屏幕绝对像素。屏幕分辨率：{w}x{h}。操作应用界面时，必须先用 read_screen 拿到按钮/菜单/输入框的中心坐标，再用 mouse_click 点击、type_text 输入、press_hotkey 按键；不知道元素位置时禁止盲点。\n\
